...
export default function LedgerDetail({
  ...
  function ledgerUpdate() {
    ...
    axios.post('api/ledger/ledgerItemUpdate', JSON.stringify(LedgerDto)).then((res) => {
      if (res.data === 'false') {
        Swal.fire({
          icon: 'error',
                      ...
                      return (
                        <div className={'ImageItem'} key={file.fileNo}>
                          <div className={'ImageCheckBox'}>
                            <Checkbox value={file.fileNo} onClick={() => CheckBoxEventHandle(file.fileNo)} />
                          </div>
                          <img className={'ledgerImg'} src={`/image/${file.filePath}`} alt={`File ${file.fileNo}`} />
                        </div>
                      );
                    })}
                  </div>

...
export default function IntegrateCategory({
  ...
  const stepOne = (
    <>
      <DialogTitle id='alert-dialog-title'>통합할 카테고리를 선택해주세요.</DialogTitle>
      <DialogContent>
        <FormGroup>
          {categoryList
            .filter((item: any) => item.delYn === 'Y')
            .map((item: any) => (
              <FormControlLabel
                key={'one' + item.categoryNo}
                control={
                  <Checkbox
                    checked={before.includes(item.categoryNo)}
                    onChange={(e) => {
                      handleBefore(e);
                    }}
                    name={String(item.categoryNo)}
                  />
                }
                label={item.content}
              />
  ...
  const stepTwo = (
    <>
      <DialogTitle id='alert-dialog-title'>어느 카테고리로 통합할까요?</DialogTitle>
      <DialogContent>
        <FormGroup>
          {categoryList
            .filter((item: any) => !before.includes(item.categoryNo))
            .map((item: any) => (
              <FormControlLabel
                key={'two' + item.categoryNo}
                control={
                  <Radio
                    checked={item.categoryNo === after}
                    onChange={(e) => {
                      handleAfter(e);
                    }}
                    name={String(item.categoryNo)}
      ...
      <DialogActions>
        ...
        <Button onClick={integrateCategory} disabled={after === 0}>
          통합
        </Button>
      </DialogActions>
    </>
